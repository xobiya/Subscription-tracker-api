...
export default function SubscriptionCard({ subscription, onEdit, onDelete }){
  ...
  const category = (subscription.category || 'other').toLowerCase()
  const initial = ((subscription.name || '?').charAt(0) || '?').toUpperCase()
  const name = subscription.name || 'Unknown'
  const price = Number(subscription.price || 0)
  const startDateText = subscription.startDate ? new Date(subscription.startDate).toLocaleDateString() : ''
          ...
          <div className="relative">
            ...
            {showMenu && (
              <div className="absolute right-0 top-10 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-10 min-w-[120px]">
                ...
                <button
                  onClick={() => { onDelete && onDelete(subscription.id); setShowMenu(false) }}
